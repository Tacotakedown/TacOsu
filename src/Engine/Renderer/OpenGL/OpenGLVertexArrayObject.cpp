...
void OpenGLVertexArrayObject::init() {
	if (!m_bAsyncReady || m_verticies.size() < 2) return;
	...
	if (m_bReady) {
		...
		if (m_partialUpdateVertexIndices.size() > 0) {
			...
			for (size_t i = 0; i < m_partialUpdateVertexIndices.size(); i++) {
				...
				glBufferSubData(GL_ARRAY_BUFFER, sizeof(Vector3) * offsetIndex, sizeof(Vector3) * numContinuousIndices, &(m_verticies[offsetIndex]));
			}
	...
	glBindBuffer(GL_ARRAY_BUFFER, m_iVertexBuffer);
	glBufferData(GL_ARRAY_BUFFER, sizeof(Vector3) * m_verticies.size(), &(m_verticies[0]), usageToOpenGL(m_usage));
...
void OpenGLVertexArrayObject::draw() {
	...
	const int start = clamp<int>(m_iDrawRangeFromIndex > -1 ? m_iDrawRangeFromIndex : nearestMultipleUp((int)(m_iNumVerticies * m_fDrawPercentFromPercent), m_iDrawPercentNearestMultiple), 0, m_iNumVerticies);
	const int end = clamp<int>(m_iDrawRangeToIndex > -1 ? m_iDrawRangeToIndex : nearestMultipleDown((int)(m_iNumVerticies * m_fDrawPercentToPercent), m_iDrawPercentNearestMultiple), 0, m_iNumVerticies);
