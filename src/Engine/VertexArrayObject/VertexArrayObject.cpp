...
void VertexArrayObject::clear() {
	m_verticies = std::vector<Vector3>();
...
void VertexArrayObject::addVertex(Vector2 v) {
	m_verticies.push_back(Vector3(v.x, v.y, 0));
	m_iNumVerticies = m_verticies.size();
}

void VertexArrayObject::addVertex(float x, float y, float z) {
	m_verticies.push_back(Vector3(x, y, z));
	m_iNumVerticies = m_verticies.size();
}
...
void VertexArrayObject::setVertex(int index, Vector2 v) {
	if (index < 0 || index >(m_verticies.size() - 1)) return;

	m_verticies[index].x = v.x;
	m_verticies[index].y = v.y;
