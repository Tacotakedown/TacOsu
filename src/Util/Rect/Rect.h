...
{
public:
	Rects(float x = 0, float y = 0, float width = 0, float height = 0, bool isCentered = false);
	Rects(Vector2 pos, Vector2 size, bool isCentered = false);
	...
	Rects intersect(const Rects& rect) const;
	bool intersects(const Rects& rect) const;
	Rects Union(const Rects& rect) const;
	...
	// operators
	Rects& operator = (const Rects& rect);
